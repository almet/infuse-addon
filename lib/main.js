...
menu.panel.port.on("update-credentials", function(credentials){
    ...
    webservice.checkCredentials(credentials["username"], 
        ...
        if (result == true){
            menu.tooltip = "connected";
        } else {
            menu.tooltip = "credentials are not valid";
        }
    });
});

// initialise the widget menu
user.get(function(credentials){
    // if the credentials does not exist, change the label and popup the widget
    credentials.forEach(function(c){
        webservice.checkCredentials(c.username, c.password, function(is_valid){
            if (is_valid == true){
                menu.tooltip = "connected";
                menu.panel.port.emit("credentials-checked", true);
            } else {
                menu.tooltip = "credentials are not valid";
                menu.panel.port.emit("credentials-checked", false);
            }
        });
    });
    if (credentials.length == 0){
        menu.tooltip = "please enter your credentials";
        menu.panel.show();
    }
